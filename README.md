...
**Status honesty** (2026-09-22): the query pipeline, chat handler (follow-ups, memory, quotas) and model fallback have run end to end against real Postgres, Neo4j and Gemini; the application SQL passes 33 integration tests; the app is deployed and answering real chat questions over HTTPS on Cloud Run, with Langfuse Cloud tracing and prompt management both live and confirmed working. **Not yet proven**: the online judge against a real model, a full offline evaluation of the golden set (it exceeds one free-tier day), and the overlap-merge safety net against a live model. MLflow has never logged a real run. Two papers have no method/dataset/metric links in the graph (entity extraction produced nothing usable), so graph-style questions can miss them. Not built: password-strength rules beyond length, email verification/reset flows, per-IP rate limiting (limits are per user), cross-process concurrency limiting (it is per process, and Cloud Run can scale to more than one instance, which the concurrency limiter does not account for), a CI/CD gate that blocks deployment when GitHub Actions tests fail (Cloud Build currently deploys on every push regardless), and per-question latency is high (57-78s measured live) with pipeline stage breakdown not yet re-profiled since the last cost/latency fixes.
